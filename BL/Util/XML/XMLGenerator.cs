...
namespace FIS.BL.Util.XML
{
    public class XMLGenerator : IXMLGenerator
    {
        public string GenerateXmlString(Message message)
        {
            ...
            List<Group> groupsA01 = message.Transactions.ElementAt(0).Groups.Where(g => g.GroupCode.Equals("A01")).ToList();
            foreach (Group group in groupsA01)
            {
        ...
        private static string AddGroup(string xmlString, Group group, Message message)
        {
                ...
                foreach(Field field in group.Fields)
                {
                 ...
                 List<Group> groups = message.Transactions.ElementAt(0).Groups.Where(g => !g.GroupCode.Equals("A01")).ToList();
